...
class Cape(ProcessingModule):
    name = "cape"
    description = "Submit the file to CAPE Sandbox."
    ...
    config = [
        ...
        },
        {
            ...
            'type': 'integer',
            'default': 5400,
            'description': 'Time in seconds that the module will wait for CAPE analysis to be over.'
        },
        {
            'name': 'wait_step',
            'type': 'integer',
    ...
    permissions = {
        'cape_access': "For users that have access to the CAPE instance. Will display a link to CAPE's analysis."
    }
    ...
    def each_with_type(self, target, file_type):
        ...
        self.results['URL'] = urljoin(self.web_endpoint, "/analysis/{}/".format(self.task_id))
    ...
    def submit_file(self, filepath, options):
        url = urljoin(self.web_endpoint, '/api/tasks/create/file/')
        fp = open(filepath, 'rb')
    ...
    def wait_for_analysis(self):
        url = urljoin(self.web_endpoint, '/api/tasks/view/{0}/'.format(self.task_id))
    ...
    def process_report(self):
        url = urljoin(self.web_endpoint, '/api/tasks/get/report/{0}/'.format(self.task_id))
    ...
    def get_memory_dump(self):
        url = urljoin(self.web_endpoint, '/api/tasks/get/procmemory/{0}/'.format(self.task_id))
        response = requests.get(url, stream=True)

        self.register_response_as('memory_dump', response)

    def get_pcap(self):
        url = urljoin(self.web_endpoint, '/api/tasks/get/pcap/{0}/'.format(self.task_id))
        response = requests.get(url, stream=True)
    ...
    def register_response_as(self, type, response, zipped=False):
        if response.status_code != 200:
            ...
            tmpdir = tempdir()
            filename = os.path.join(tmpdir, 'cape_response')
            f = open(filename, "wb")
